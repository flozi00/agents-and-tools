...
class Tools:
    class Valves(BaseModel):
        ...
        allowed_law_codes: str = Field(
            ...
            description=(
                "Kommaseparierte Liste zulässiger Gesetzeskürzel, z. B. 'bgb,gg,sgb_12'. "
                "Nur diese Kürzel werden abgerufen bzw. aus Suchtreffern zurückgegeben."
            ),
        )
        ...
        max_output_chars: int = Field(
            ...
            ge=0,
            description=(
                "Optionale Ausgabelängenbegrenzung. 0 bedeutet unbegrenzt. "
                "Wenn begrenzt, wird JSON am Ende mit einem Hinweis gekürzt."
            ),
        )
        toc_url: str = Field(
    ...
    async def getLaw(
        ...
        await self._emit_status(
            __event_emitter__, f"getLaw gestartet: {law_code}", done=False
        )
        try:
            normalized_law = self._validate_law_code(law_code)
            requested_refs = self._normalize_requested_refs(paragraphs or [])
            mode = (
                "fulltext" if fulltext else ("paragraphs" if requested_refs else "toc")
            )
            ...
            try:
                ...
                if "404" in str(download_exc):
                    ...
                    raise
            xml_bytes, xml_filename = await asyncio.to_thread(
                self._extract_xml_from_zip, zip_bytes
            )
            ...
            if fulltext:
                ...
                selected = [
                    norm for norm in norms if self._norm_matches(norm, requested_refs)
                ]
                payload = {
                    "law_code": normalized_law,
                    "source_url": self._law_zip_url(normalized_law),
                ...
                if not selected:
                    payload["warning"] = (
                        "Keine passenden Paragraphen/Artikel im Gesetzestext gefunden."
                    )
            else:
                toc = [
                    {
            ...
            await self._emit_status(
                __event_emitter__, f"getLaw abgeschlossen: {normalized_law}", done=True
            )
            return result
        except Exception as exc:  # bewusst defensiv für Tool-Aufruf-Kontext
            await self._emit_status(
                __event_emitter__, f"getLaw fehlgeschlagen: {exc}", done=True
            )
            return self._to_json(
                {"error": str(exc), "tool": "getLaw", "law_code": law_code}
            )
    ...
    async def searchLaw(
        ...
        await self._emit_status(
            __event_emitter__, f"searchLaw gestartet: {safe_query}", done=False
        )
        try:
            if not safe_query:
            ...
            payload = {
                ...
                "allowed_law_codes": (
                    "*"
                    if self.valves.allow_all_laws
                    else sorted(self._allowed_law_codes())
                ),
                "results": results,
            }
            await self._emit_debug(
            ...
            await self._emit_status(
                __event_emitter__, f"searchLaw fehlgeschlagen: {exc}", done=True
            )
            return self._to_json(
                {"error": str(exc), "tool": "searchLaw", "query": query}
            )
    ...
    async def findLaw(
        ...
        safe_query = (query or "").strip()
        await self._emit_status(
            __event_emitter__, f"findLaw gestartet: {safe_query}", done=False
        )
        try:
            if not safe_query:
                raise ValueError("query darf nicht leer sein.")
            ...
            for key_norm, slug in toc.items():
                ...
                if normalized_query and normalized_query in key_norm:
                    seen.add(slug)
                    results.append(
                        {"law_code": slug, "xml_url": self._law_zip_url(slug)}
                    )
            ...
            await self._emit_status(
                __event_emitter__, f"findLaw fehlgeschlagen: {exc}", done=True
            )
    ...
    async def _load_toc(self) -> Dict[str, str]:
        now = time.time()
        if (
            self._toc_cache is not None
            and (now - self._toc_cache_ts) < self.valves.toc_cache_seconds
        ):
    ...
    def _parse_toc(self, xml_text: str) -> Dict[str, str]:
        ...
        for item in root.iter():
            ...
            mapping.setdefault(
                self._normalize_code(self._strip_version_suffix(slug)), slug
            )
            if title:
    ...
    async def _resolve_slug(self, law_code: str) -> str:
        ...
        if key in toc:
            return toc[key]
        stripped = self._normalize_code(
            self._strip_version_suffix((law_code or "").strip().lower())
        )
        if stripped in toc:
            return toc[stripped]
        raise ValueError(
            f"Gesetzeskürzel '{law_code}' nicht im Inhaltsverzeichnis "
            f"(gii-toc.xml) gefunden. Bitte Kürzel oder vollständigen Titel prüfen."
        )
    ...
    def _validate_law_code(self, law_code: str) -> str:
        ...
        if not re.fullmatch(r"[a-z0-9_-]{1,64}", normalized):
            raise ValueError(
                "Ungültiges Gesetzeskürzel. Erlaubt sind a-z, 0-9, '_' und '-'."
            )
        if (
            not self.valves.allow_all_laws
            and normalized not in self._allowed_law_codes()
        ):
            raise ValueError(
                f"Gesetzeskürzel '{normalized}' ist nicht in allowed_law_codes konfiguriert."
            )
        return normalized
    ...
    def _allowed_law_codes(self) -> Set[str]:
        return {
            part.strip().lower()
            for part in (self.valves.allowed_law_codes or "").split(",")
            if part.strip()
        }
    ...
    async def _download_law_zip(self, law_code: str) -> bytes:
        ...
        if len(data) > self.valves.max_zip_bytes:
            raise ValueError(
                f"ZIP-Datei ist zu groß ({len(data)} Bytes > {self.valves.max_zip_bytes} Bytes)."
            )
        return data
    ...
    def _download_bytes_sync(self, url: str) -> bytes:
        ...
        ) as response:  # nosec: URL ist Valve/validiert
            content_length = response.headers.get("Content-Length")
            if content_length and int(content_length) > self.valves.max_zip_bytes:
                raise ValueError(
                    f"Antwort ist zu groß ({content_length} Bytes > {self.valves.max_zip_bytes} Bytes)."
                )
    ...
    def _extract_norms(self, root: ET.Element) -> List[Dict[str, Any]]:
        ...
        for index, norm_el in enumerate(
            self._iter_by_local_name(root, "norm"), start=1
        ):
            ...
            norm_id = (
                norm_el.attrib.get("builddate")
                or norm_el.attrib.get("doknr")
                or str(index)
            )
    ...
    def _extract_search_results(
        self, anchors: Iterable[_SearchAnchor], limit: int
    ) -> List[Dict[str, Any]]:
        results: List[Dict[str, Any]] = []
        ...
        for anchor in anchors:
            ...
            if (
                not self.valves.allow_all_laws
                and law_code not in self._allowed_law_codes()
            ):
                continue
    ...
    def _first_child(
        self, element: ET.Element, local_name: str
    ) -> Optional[ET.Element]:
