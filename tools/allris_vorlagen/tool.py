...
class EventEmitter:
    ...
    async def debug(self, title: str, data: Any, enabled: bool) -> None:
        ...
        try:
            ...
            content = f'{title}:\n{data}'
        await self.emitter({'type': 'notification', 'data': {'type': 'info', 'content': content}})
...
class Tools:
    class Valves(BaseModel):
        ...
        allow_redirects: bool = Field(
            default=False,
            description=('Erlaubt HTTP-Redirects bei ALLRIS-Aufrufen. Aus Sicherheitsgründen standardmäßig deaktiviert, passend zur OpenWebUI-0.9.5-SSRF-Härtung gegen redirect-basierte Angriffe.'),
        )
        allowed_hosts: str = Field(
            default='',
            description=('Optionale kommagetrennte Host-Allowlist für ALLRIS-URLs. Wenn leer, werden die Hosts aus public_base_url und private_base_url verwendet.'),
        )
        ...
        prompt_summary: str = Field(
            default=('Fasse die folgende kommunale Vorlage präzise zusammen. Berücksichtige ausdrücklich Beschlussvorschlag, Beschlusstexte, Beratungsergebnisse, Betreff, Sachverhalt und relevante Dokumente. Gib die Antwort auf Deutsch als Markdown aus.'),
    ...
    async def search_vorlagen(
        ...
        await events.status(f'🔎 Suche ALLRIS-Vorlagen nach: {query} ({"nicht öffentlich" if use_private_access else "öffentlich"})')
        try:
            async with self._client() as client:
                login_ok = await self._login_if_enabled(client, events, __event_call__, __user__, base_url, use_private_access)
    ...
    async def summarize_vorlage(
        ...
        base_url = self._base_url(use_private_access)
        try:
            await events.status(f'📖 Lade Vorlage für Zusammenfassung ({"nicht öffentlich" if use_private_access else "öffentlich"}) ...')
            async with self._client() as client:
                login_ok = await self._login_if_enabled(client, events, __event_call__, __user__, base_url, use_private_access)
                if use_private_access and not login_ok:
                    return 'Login erforderlich. Es konnte keine gültige ALLRIS-Session aufgebaut werden.'
        ...
        response = f'# Zusammenfassung: {doc.title or "ALLRIS Vorlage"}\n\n**URL:** {doc.url}\n\n{summary}\n\n---\n\n## Beschlusstexte\n\n{decision_text}'
    ...
    async def get_vorlage_text(
        ...
        base_url = self._base_url(use_private_access)
        try:
            await events.status(f'📖 Lade Gesamttext der Vorlage ({"nicht öffentlich" if use_private_access else "öffentlich"}) ...')
            async with self._client() as client:
                login_ok = await self._login_if_enabled(client, events, __event_call__, __user__, base_url, use_private_access)
                if use_private_access and not login_ok:
                    return 'Login erforderlich. Es konnte keine gültige ALLRIS-Session aufgebaut werden.'
    ...
    async def get_vorlage_anlagen(
        ...
        try:
            await events.status(f'📎 Lade Anlagen der Vorlage ({"nicht öffentlich" if use_private_access else "öffentlich"}) ...')
            async with self._client() as client:
                login_ok = await self._login_if_enabled(client, events, __event_call__, __user__, base_url, use_private_access)
                if use_private_access and not login_ok:
                    return 'Login erforderlich. Es konnte keine gültige ALLRIS-Session aufgebaut werden.'
                doc = await self._fetch_template_document(client, vorlage.strip(), events, base_url)
                attachments = self._collect_attachments(doc, include_agenda_documents=include_agenda_documents)
        ...
        file_events = [{'name': item['name'], 'url': item['url']} for item in attachments if item.get('url')]
        ...
        if use_private_access:
            lines.append('> Hinweis: Die Anlagen stammen aus dem nicht öffentlichen Bereich. Der direkte Aufruf der Links kann außerhalb der aktuellen ALLRIS-Session eine erneute Anmeldung erfordern.')
            lines.append('')
    ...
    async def _login_if_enabled(
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            await events.status(f'❌ ALLRIS-Login fehlgeschlagen ({attempt}/{max_attempts}).' + (' Bitte Zugangsdaten erneut eingeben.' if attempt < max_attempts else ''))
    ...
    def _extract_document_items(self, soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
        ...
        selectors = "aside#dokumenteHeaderPanel a[href], a.pdf[href], a[href$='.pdf'], a[href*='getfile'], a[href*='doctoshow'], a[href*='document']"
    ...
    def _collect_attachments(self, doc: TemplateDocument, include_agenda_documents: bool = True) -> List[Dict[str, str]]:
        ...
        for index, item in enumerate(attachments, start=1):
            ...
            try:
                ...
                if len(content) > self.valves.max_attachment_download_bytes:
                    item['text_error'] = f'PDF wurde nicht verarbeitet, weil die Datei größer als {self.valves.max_attachment_download_bytes} Bytes ist.'
                    continue
    ...
    def _extract_pdf_text(self, content: bytes) -> str:
        try:
            ...
            return f"PDF-Text konnte nicht extrahiert werden, weil pypdf nicht verfügbar ist. Installiere die Function-Anforderung 'pypdf'. ({type(exc).__name__}: {exc})"
    ...
    def _validate_allris_url(self, url: str, base_url: str) -> str:
        ...
        if hostname not in allowed_hosts:
            raise ValueError(f"Host '{hostname}' ist nicht in der ALLRIS-Allowlist ({', '.join(sorted(allowed_hosts))}) enthalten.")
        return url
    ...
    async def _select_private_access(
        ...
        if cached is not None:
            await events.status('🔐 Nutze für diesen Chat weiterhin den nicht öffentlichen ALLRIS-Bereich.' if cached else '🌐 Nutze für diesen Chat weiterhin den öffentlichen ALLRIS-Bereich.')
            return cached

        if not event_call:
            await events.status('ℹ️ Keine interaktive Bereichsauswahl verfügbar – nutze öffentlichen ALLRIS-Bereich.')
        ...
        if session:
            expires_at = float(session.get('expires_at') or 0)
            session_base_url = session.get('base_url') or ''
            has_private_session = expires_at > time.time() and session_base_url == self.valves.private_base_url.rstrip('/')

        message = 'Soll der nicht öffentliche ALLRIS-Bereich durchsucht werden?\n\nBestätigen: nicht öffentlicher Bereich mit Login/private_base_url.\nAbbrechen: öffentlicher Bereich ohne Login/public_base_url.'
        ...
        await events.status('🔐 Nicht öffentlicher ALLRIS-Bereich für diesen Chat gespeichert.' if use_private else '🌐 Öffentlicher ALLRIS-Bereich für diesen Chat gespeichert.')
        return use_private

    def _get_chat_access_choice(self, user: Optional[dict], metadata: Optional[dict]) -> Optional[bool]:
    ...
    def _save_chat_access_choice(self, user: Optional[dict], metadata: Optional[dict], use_private: bool) -> None:
        chat_key = self._chat_key(user, metadata)
        if not chat_key:
            return
    ...
    def _chat_key(self, user: Optional[dict], metadata: Optional[dict]) -> str:
        ...
        if isinstance(metadata, dict):
            chat_id = str(metadata.get('chat_id') or metadata.get('chatId') or metadata.get('conversation_id') or metadata.get('session_id') or '')
