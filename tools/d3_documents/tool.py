...
class Tools:
    class Valves(BaseModel):
        ...
        FILE_REFERENCE_PROPERTY_KEY: str = Field(
            default='',
            description=('Optional d.3 source property key containing the Aktenzeichen/file reference. If set, getFileByReference searches this property explicitly. If empty, it uses fulltext.'),
        )
        ...
        SEARCH_PROPERTY_KEYS: str = Field(
            default='',
            description=('Optional comma-separated d.3 source property keys. If set, searchDocuments sends sourceproperties={key:[query]}. If empty, it uses the fulltext parameter as plain text.'),
        )
