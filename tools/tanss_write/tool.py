...
class Tools:
    class Valves(BaseModel):
        ...
        mail_recipient_domains: str = Field(
            ...
            description=(
                "Optional comma-separated allowlist of recipient email domains "
                "for send_ticket_mail (e.g. 'example.com,partner.de'). Empty "
                "means any recipient is allowed."
            ),
        )
    ...
    def _request(
        ...
        *,
        ...
        if normalized_method not in allowed_methods:
            raise ValueError(
                f"Method {normalized_method} is not allowed. This tool only "
                "performs create/update operations; use the TANSS read tool "
                "for reads."
            )
    ...
    def _check_mail_recipients(self, *address_fields: str) -> None:
        allowed = {
            ...
            for domain in (self.valves.mail_recipient_domains or "")
            .replace(";", ",")
            .split(",")
            if domain.strip()
        }
        ...
        for field in address_fields:
            for address in re.split(r"[,;]", field or ""):
                ...
                if domain not in allowed:
                    raise ValueError(
                        f"Recipient {address!r} is not in the allowed mail "
    ...
    def create_ticket(
        ...
        return self._request(
            "POST", "/api/v1/tickets", params=params, json_body=payload
        )
    ...
    def send_ticket_mail(
        ...
        if normalized_type not in {
            ...
            raise ValueError(
                "Invalid mail_type. Allowed: NORMAL, REPLY, SEND_AGAIN, "
                "FORWARD, SEMI_AUTOMATE"
            )
        if not (to or "").strip():
    ...
    def create_support(
        ...
        if ticket_id is None and not (extra_fields or {}).get("linkId"):
            raise ValueError(
                "Either ticket_id or extra_fields with linkTypeId/linkId is required."
            )
        payload = self._build_payload(
            {
                "date": date,
